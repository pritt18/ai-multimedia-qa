...
function TimestampSearch({ fileId, mediaUrl }) {
  ...
  const searchTopic = async () => {
    try {
      ...
      alert("Error searching timestamps");
    }
  };

  const handlePlay = (startTime) => {
    // Force React state update even if same timestamp clicked again
    setSelectedTime(startTime + Math.random() * 0.001);
  };
      ...
      {timestamps.length > 0 && (
        <div className="mt-6 bg-gray-700 p-5 rounded-lg">
          ...
          {timestamps.map((item, index) => (
            ...
            >
              <p>
                <strong>Start:</strong> {item.start.toFixed(2)} sec
              </p>

              <p>
                <strong>End:</strong> {item.end.toFixed(2)} sec
              </p>
